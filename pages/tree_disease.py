...
register_page(__name__, path='/tree_disease',
              title='Tree Disease', name='Tree Disease')
...
layout = html.Div([
    html.Div([
        ...
        html.Div(id='output-filename-tree'),
    ]),
    html.Div([
        html.Button('Submit', id='submit-file-btn-tree', n_clicks=0),
    ], className="process-btn-div"),
    html.Div(id='model_output-tree', className="model-output-div"),
], className="twelve columns page_div")


# ------------- Callback Methods ----------------
@callback(Output('output-filename-tree', 'children'),
          Input('upload-data-tree', 'contents'),
          State('upload-data-tree', 'filename'),
          State('upload-data-tree', 'last_modified'),
...
model = load_model(r'PalmDiseases.h5')  # Load the saved model
...
@callback([Output('model_output-tree', 'children'),
           ],
          Input('submit-file-btn-tree', 'n_clicks'),
          State('upload-data-tree', 'contents'),
          State('upload-data-tree', 'filename'),
          State('upload-data-tree', 'last_modified'),
          prevent_initial_call=True
          )
